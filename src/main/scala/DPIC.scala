...
class DPICBatch(template: Seq[DifftestBundle], batchIO: BatchIO, config: GatewayConfig) extends DPICBase(config) {
  ...
  override def dpicFuncAssigns: Seq[String] = {
    ...
    Seq(s"""
           ...
           |    if (id >= ${bundleEnum.length}) {
           |      printf("Batch bundle id out of range: id %u, max %u\\n", id, ${bundleEnum.length - 1});
           |      assert(0);
           |    }
           |#ifdef CONFIG_DIFFTEST_QUERY
           |    if (qStats) {
           ...
           |    $bundleAssign
           |    else {
           |      printf("Unknown Batch bundle id: %u\\n", id);
           |      assert(0);
           |    }
           |  }
           |  if (!batch_finished) {
           |    printf("Batch packet has no finish marker\\n");
           |    assert(0);
           |  }
           |""".stripMargin)
  }
