...
case class BatchParam(config: GatewayConfig, bundles: Seq[DifftestBundle]) {
  ...
  if (!config.batchSplit) {
    require(
      StepDataByteLen <= MaxDataByteLen,
      s"Batch step data requires $StepDataByteLen bytes, but only $MaxDataByteLen bytes are configured",
    )
    require(
      StepInfoByteLen <= MaxInfoByteLen,
      s"Batch step info requires $StepInfoByteLen bytes, but only $MaxInfoByteLen bytes are configured",
    )
  }
...
class BatchAssembler(
  ...
  if (config.batchSplit) {
    ...
    assert(!delay_step_enable || delay_step_status.last.data_bytes <= param.MaxDataByteLen.U)
    assert(!delay_step_enable || delay_step_status.last.info_size <= param.MaxInfoSize.U)
