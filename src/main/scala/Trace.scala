...
//    val traceNameProto = "extern void \"C\" set_iotrace_name(char *s)"
//    difftestCpp += traceNameProto + ";"
    ...
    val (isRead, traceFunc, preInit) = if (isDump) {
      ("false", "append", "")
    } else {
      ("true", "read_next", "memset(io, 0, sizeof(DifftestIOTrace));")
    }
    difftestCpp +=
      s"""
